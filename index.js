...
function removeHTMLTags(str) {
    new doc = new DOMParser().parseFromString(input, 'text/html');
...
function getGamesByMonth(year, month) {
    ...
    fetch(`https://api.rawg.io/api/games?dates=${year}-${month + 1}-01,${year}-${month + 1}-30&key=${API_KEY}`)
        .then((response) => {
        ...
        .then((data) => {
            ...
            document.querySelectorAll('.game').forEach((gameEl) => {
                gameEl.addEventListener('click', () => {
                    ...
                    fetch(`https://api.rawg.io/api/games/${gameId}?key=${API_KEY}`).then((response) => {return response.json();}).then((gameDetails) => {
                            ...
                            descriptionElement.nextSibling.nodeValue = " " + stripHtmlTags(gameDetails.description || "No description available for this game.");
